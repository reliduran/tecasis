...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['actualizar_nosotros'])) {
    ...
    $email_c = $conn->real_escape_string($_POST['email_contacto']);
    $tel_c   = $conn->real_escape_string($_POST['telefono']);
    $dir_c   = $conn->real_escape_string($_POST['direccion']);
    $hor_c   = $conn->real_escape_string($_POST['horarios']);
    ...
    $sql = "UPDATE seccion_nosotros SET 
            ...
            email_contacto='$email_c', telefono='$tel_c', 
            direccion='$dir_c', horarios='$hor_c' 
            $img_sql 
            WHERE id=1";
...
<form method="POST" enctype="multipart/form-data">
   ...
   <!-- Nuevos campos para contacto y horarios -->
    <div class="input-group"><label>Email de Contacto:</label><input type="email" name="email_contacto" value="<?php echo htmlspecialchars($nosotros_data['email_contacto']); ?>"></div>
    <div class="input-group"><label>Teléfono:</label><input type="text" name="telefono" value="<?php echo htmlspecialchars($nosotros_data['telefono']); ?>"></div>
    <div class="input-group"><label>Dirección Física:</label><textarea name="direccion"><?php echo htmlspecialchars($nosotros_data['direccion']); ?></textarea></div>
    <div class="input-group"><label>Horarios de Atención:</label><textarea name="horarios"><?php echo htmlspecialchars($nosotros_data['horarios']); ?></textarea></div>
